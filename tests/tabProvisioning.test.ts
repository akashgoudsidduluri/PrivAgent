...
describe('Target Tab Provisioning', () => {
  ...
  describe('resolveTargetWebTab — existing tab wins', () => {
    it('1. returns the existing eligible tab and never proposes provisioning', () => {
      const webTab: MinimalTab = { id: 42, url: 'https://example.org/portal', active: false };
      const result = resolveTargetWebTab([DASHBOARD, webTab], 'open google and search for black cats');
