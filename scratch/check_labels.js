async function main() {
  ...
  await sendCommand('Runtime.evaluate', {
    expression: `chrome.tabs.reload(${googleTabId})`,
    awaitPromise: true,
  }, swSession);
  ...
  console.log('Total detections:', scanRes.result?.value?.report?.detections?.length);
  for (const d of scanRes.result?.value?.report?.detections || []) {
    if (d.label || d.type === 'input' || d.type === 'search' || d.type === 'button') {
      console.log(` - [${d.id}] type=${d.type} label="${d.label}" selector="${d.selector}" bbox=[${d.bbox}]`);
    }
  }
