...
| **2. PII Detection Precision / Recall / F1** | **20%** | **F1: 0.56 (Micro) / 0.38 (Macro)** | Tested on labelled synthetic dataset (`evaluation/datasets/pii_benchmark_dataset.json`) across 9 sensitive categories + 8 negative controls. Precision: 85.0% Macro / 64.3% Micro. |
...
| **4. Client Resource Utilization** | **20%** | **P50: 3.19 ms / 61.6 MB** | Profiling across 10 complete perception cycles on AMD Ryzen 5: DOM Scan P50: 2.49ms, Fusion P50: 0.12ms, Minimization P50: 0.40ms. Mean Full Cycle: 3.42ms. |
...
1. **DOM & Visual Redaction**: Passwords, OTPs, CVVs, PANs, card numbers, and account numbers are identified and masked directly in memory before context serialization.
...
## 🛡️ Adversarial & Security Hardening (Phase 8)

PrivAgent has been verified against 10 critical security vectors:
...
```bash
# Frontend & Extension Tests (33 suites, 353 tests)
npm test
...
| **TEST 5** | `"Open the localhost 4173 and get my account number"` | Resolves `:4173`, perceives DOM/OCR, masks 6 sensitive fields, sends sanitized context. |
